...
class ImageView(tk.Canvas):
    ...
    def _photo_from_rgb(self, rgb: np.ndarray) -> tk.PhotoImage:
    ...
    def set_image(self, img_rgb: Optional[np.ndarray]) -> None:
        self._img_rgb = img_rgb
        if img_rgb is not None:
            self._img_h, self._img_w = img_rgb.shape[:2]
        else:
    ...
    def _fit(self) -> None:
        ...
        if self._img_rgb is None or self._img_w <= 0 or self._img_h <= 0:
    ...
    def _ensure_scaled_base_image(self) -> None:
        ...
        im = cv2.resize(self._img_rgb, (w, h), interpolation=interp)
        self._img_tk = self._photo_from_rgb(im)
    ...
    def _on_move(self, e) -> None:
        if not self._drag or self._img_rgb is None:
            return
